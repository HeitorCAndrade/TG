...
%           [ reference, fs ] = audioread( 'speech.wav' );
%           [ degraded, fs ] = audioread( 'speech_bab_0dB.wav' );
% 
...
function [ scores ] = pesqbin_linux( reference, degraded, fs, mode, binary )
    ...
    if isstr(reference)
        ...
        [ ~, fs, nbits ] = audioread( file.reference );
    else
        file.reference = sprintf( '%s%s%s', tmpdir, filesep, '~80b4eb734d.wav' );
        audiowrite( file.reference, 0.999*reference./max(abs(reference)), fs, 'BitsPerSample', nbits );
    end % isstr(reference)
    ...
    if isstr(degraded)
        ...
        [ ~, fs, nbits ] = audioread( file.degraded );
    else
        file.degraded = sprintf( '%s%s%s', tmpdir, filesep, '~be4dfad7fba.wav' );
        audiowrite( file.degraded, 0.999*degraded./max(abs(degraded)), fs, 'BitsPerSample', nbits );
    end % isstr(degraded)
...
function [ scores ] = pesqbin_windows( reference, degraded, fs, mode, binary )
    ...
    %[ ~, tmpdir ] = system( 'echo C:\Users\SONY\Costa\Temp' ); % e.g., C:\DOCUME~1\USERNAME\LOCALS~1\Temp
    %tmpdir = regexprep( tmpdir, '\n.*', '' );
    tmpdir = pwd();     % Em 28/06/2013 substitui as duas linhas anteriores por essa. Marcio Costa
    expdir = pwd();
